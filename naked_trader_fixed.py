"""
NAKED TRADER FIXED — Proven patterns + chart gate + top 25 coins
...
EXCLUDED = {'PAXG/USD'}
TOP_COINS = {'BTC/USD','ETH/USD','SOL/USD','BNB/USD','XRP/USD','AVAX/USD','LINK/USD','FET/USD',
             'TAO/USD','APT/USD','SUI/USD','NEAR/USD','PENDLE/USD','ADA/USD','DOT/USD',
             'UNI/USD','HBAR/USD','AAVE/USD','CAKE/USD','DOGE/USD','FIL/USD','LTC/USD',
             'SEI/USD','ARB/USD','ENA/USD'}
...
def check_entries(td):
    ...
    candidates = []
    for pair, info in td.items():
        if pair in EXCLUDED or pair in positions or pair not in TOP_COINS: continue
        ...
        # CHART GATE: no chart pattern = need score >= 8
        if not best_chart and candle_score < 8: continue
...
def main():
    ...
    alert(
        '<b>🎯 NAKED TRADER V9</b>\n'
        '10 proven patterns + chart gate + top 25 coins\n'
